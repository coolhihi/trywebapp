...
function showLoading(){
    loadingLayout.fadeIn(200);
}
function hideLoading(){
    setTimeout(function(){loadingLayout.hide();},400);
}
 ...
 */
globalurl='';
function oLoad(url,method,data,way,easing,during,historyflag,beforeFunc,afterFunc){
    if(!url || url==='#')
        return false;
    showLoading();
    globalurl=url;
            ...
            try{
                ...
                if(result!=null && result.length>0){
                    ...
                    if(way===0){
                        ...
                        var $newPage=$(newPage);
                        $newPage.css({left:'0px',top:'-110%'});
                        $newPage.html(result[0]);
                        $newPage.animate({left:'0px',top:'0px'},during,easing,function(){
                            $('#MainShow').remove();
                            $('#ComingShow').attr('id','MainShow');
                        });
                    }
                    else if(way===2){
                        //右方切入
...
function oReload(){
    ...
    return oLoad(nowurl,'get',{},0,'ease-out',400,false,null,null);
}
...
function oGoback(url){
    return oLoad(url,'get',{},4,'ease-out',400,true,null,null);
}
 ...
 */
function dosearch(){
    return oLoad('index.html','get',{},0,'ease-out',400,false,null,null);
}
//**************************************************************************************
//函数/方法集合 - 结束
//**************************************************************************************
...
$(document).on('click','a.pjwrap',function(){
    ...
    return oLoad(url,'get',{},0,'ease-out',400,false,null,null);
});
$(document).on('click','a.pjtop',function(){
    var url=$(this).attr('href');
    return oLoad(url,'get',{},1,'ease-out',400,false,null,null);
});
$(document).on('click','a.pjright',function(){
    var url=$(this).attr('href');
    return oLoad(url,'get',{},2,'ease-out',400,false,null,null);
});
$(document).on('click','a.pjbottom',function(){
    var url=$(this).attr('href');
    return oLoad(url,'get',{},3,'ease-out',400,false,null,null);
});
$(document).on('click','a.pjleft',function(){
    var url=$(this).attr('href');
    return oLoad(url,'get',{},4,'ease-out',400,false,null,null);
});
//**************************************************************************************
//框架重点delegate监控集合 - 结束
//**************************************************************************************
